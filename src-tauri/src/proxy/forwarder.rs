...
use super::{
    ...
    providers::{
        get_adapter, resolve_custom_request_headers, AuthInfo, AuthStrategy, ProviderAdapter,
        ProviderType,
    },
...
impl RequestForwarder {
    ...
    async fn forward(
        ...
        let custom_accept_encoding = custom_headers.remove("accept-encoding");
        ...
        if let Some(value) = custom_headers.remove("authorization") {
            auth_headers.retain(|(name, _)| !name.eq("authorization"));
            auth_headers.push((http::header::AUTHORIZATION, value));
        }
        if let Some(value) = custom_headers.remove("x-api-key") {
            auth_headers.retain(|(name, _)| !name.eq("x-api-key"));
            auth_headers.push((http::HeaderName::from_static("x-api-key"), value));
        }
        if let Some(value) = custom_headers.remove("x-goog-api-key") {
            auth_headers.retain(|(name, _)| !name.eq("x-goog-api-key"));
            auth_headers.push((http::HeaderName::from_static("x-goog-api-key"), value));
        }
        if let Some(value) = custom_headers.remove("chatgpt-account-id") {
            auth_headers.retain(|(name, _)| !name.eq("chatgpt-account-id"));
            auth_headers.push((
        ...
        let anthropic_beta_value = if adapter.name() == "Claude" {
            ...
            Some(if let Some(beta) = custom_anthropic_beta
                .as_ref()
        ...
        for (key, value) in headers {
            ...
            if key_str.eq_ignore_ascii_case("accept-encoding") {
                if !saw_accept_encoding {
                    ...
                    } else if let Some(custom_value) = custom_accept_encoding.as_ref() {
                        ordered_headers.append(key.clone(), custom_value.clone());
                    } else {
            ...
            if key_str.eq_ignore_ascii_case("anthropic-version") {
                ...
                if let Some(custom_value) = custom_anthropic_version.as_ref() {
                    ordered_headers.append(key.clone(), custom_value.clone());
                } else {
                    ordered_headers.append(key.clone(), value.clone());
                }
            ...
            // --- 默认：透传 ---
            if let Some(custom_value) = custom_headers.remove(key.clone()) {
                ordered_headers.append(key.clone(), custom_value);
            } else {
                ordered_headers.append(key.clone(), value.clone());
            }
        }
        ...
        } else if !saw_accept_encoding {
            if let Some(custom_value) = custom_accept_encoding {
                ordered_headers.append(http::header::ACCEPT_ENCODING, custom_value);
            }
        }
        ...
        if adapter.name() == "Claude" && !saw_anthropic_version {
            if let Some(custom_value) = custom_anthropic_version {
                ordered_headers.append("anthropic-version", custom_value);
            } else {
                ordered_headers.append(
                    "anthropic-version",
